...
6. While recording, open **Transcript** for recognized passages and partial speech previews, or **Study notes** for streamed note drafts after enough transcript context accumulates. A missing speech model is shown in the lecture status; choose its folder in App setup and restart services after finishing recording. Stop recording when the lecture ends; local processing continues over remaining saved audio.
