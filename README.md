...
3. Choose an existing speech-model folder (the selector offers discovered local models), start services and select **Open workspace**, then choose an installed local note model in the lecture’s note settings. Models are never downloaded by Notetaker. Restart standalone services after changing the speech folder.
