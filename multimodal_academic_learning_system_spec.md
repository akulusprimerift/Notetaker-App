...
Scope update (2026-09-11): user-requested visual notes and both API/subscription connections are brought forward into the active M08 increment. Model-written source-linked schematics render through native Qt SVG and inert HTML export; arbitrary model HTML/JavaScript is never executed. Optional OpenAI/Claude API keys and official-client subscription bridges require explicit per-lecture cloud consent. Audio/speech remain local. Existing history, edits and final snapshots are retained. See [implementation and provider qualification limits](docs/implementation/visual-notes-providers.md); this supersedes the earlier fully-local-only and Phase-7-only provider scope below.
...
## User-selected native Windows profile — 2026-09-09

The user's explicit browser-free requirement supersedes the earlier Electron delivery decision. The native application uses Qt Widgets and Qt Multimedia, with no WebEngine, Chromium or WebView. It reuses authenticated FastAPI domain contracts in private local child processes. Bundled Python, speech and CPU Ollama runtimes remove separate application prerequisites; model weights require explicit local selection. Standalone SQLite WAL with FULL synchronous writes and immutable verified private audio files is an explicitly selected Windows profile, not the preview mode. PostgreSQL/SeaweedFS/Kafka remain the existing development-service profile. Database polling reconciles standalone work without a broker. Existing Docker student data is not migrated or altered implicitly. Clean-machine, physical audio, endurance and educational-quality qualification remain open.
