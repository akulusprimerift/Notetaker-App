...
Scope update (2026-09-11): user-requested visual notes and both API/subscription connections are brought forward into the active M08 increment. Model-written source-linked schematics render through the Electron React/SVG reader and inert HTML export; arbitrary model HTML/JavaScript is never executed. Optional OpenAI/Claude API keys and official-client subscription bridges require explicit per-lecture cloud consent. Audio/speech remain local. Existing history, edits and final snapshots are retained. See [implementation and provider qualification limits](docs/implementation/visual-notes-providers.md); this supersedes the earlier fully-local-only and Phase-7-only provider scope below.
...
Latest Windows direction (2026-09-11): Electron is the only Windows host. The hardened shell uses the React renderer and existing Docker/FastAPI services. Local browser capture provides device selection, independent captured/verified progress, real segment previews and streamed notes, and individual lecture deletion. Note models remain local user selections; no model weights are downloaded. This supersedes the removed Qt/native direction below.
...
- Product domain: any course or subject, clarified by the user on 2026-09-08. Derive the subject and context from the transcript and honor the student’s writing preferences. Computer science, algorithms and code remain the initial evaluation anchor; biology and history tests now cover other subjects. English, one student's private workspace and 45–60 minute endurance fixtures are initial assumptions, not permanent course/language/duration limits.
...
The user superseded the Qt/native direction and selected Electron as the only Windows host. The app reuses the authenticated FastAPI domain contracts, Docker service lifecycle, browser capture journal, source validation and local model discovery. The installer is a development distribution with disclosed Docker, PowerShell, Ollama and speech-model prerequisites; existing Docker student data is preserved and only reused through an explicit workspace choice. Clean-machine, physical audio, endurance, accessibility, coordinated restore and educational-quality qualification remain open.
