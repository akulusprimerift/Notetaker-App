...
const statuses:Record<string,string>={listening:'Listening · waiting for the next saved speech window',not_started:'No saved speech yet',awaiting_saved_audio:'Waiting for recording to finish saving',queued:'Waiting for local transcription',processing:'Transcribing saved audio',needs_attention:'Transcription needs attention',processed:'Saved audio processed'};
...
export default function Transcript({owner,lecture,csrf,onBusy,onSessionExpired}:{owner:string;lecture:string;csrf:string;onBusy:(value:boolean)=>void;onSessionExpired:()=>void}){
  ...
  const [reading,setReading]=useState<Snapshot|null>(null);
  useEffect(()=>{if(data?.snapshot)setReading(previous=>!previous||extendsReading(previous,data.snapshot!)?data.snapshot:previous);},[data]);
  const shown=reading??data?.snapshot;
  const pending=reading&&data?.snapshot&&!extendsReading(reading,data.snapshot)?data.snapshot:null;
  ...
  const refresh=useCallback(async()=>{
    ...
    if(token===generation.current){setData(result);setConnectionError('');}
  },[base]);
  useEffect(()=>{
    let active=true;
    const load=async()=>{try{if(active)await refresh();}catch(err){if(active){if(err instanceof TranscriptError&&err.status===401)onSessionExpired();else setConnectionError(err instanceof Error?err.message:'The connection is unavailable.');}}};
    const live=(event:Event)=>{const detail=(event as CustomEvent).detail;if(detail.lecture===lecture){++generation.current;setData(detail.snapshot.transcript);setConnectionError('');}};
    window.addEventListener('lecture-snapshot',live);
    void load();const interval=setInterval(()=>void load(),4000);
    return()=>{active=false;++generation.current;clearInterval(interval);window.removeEventListener('lecture-snapshot',live);};
  },[refresh,onSessionExpired,lecture]);
  useEffect(()=>{
  ...
  async function save(){
    ...
    try{
      ...
      setDraft(null);setMessage('Correction saved as a new revision. The original words remain in passage history.');await refresh();setReading(null);
