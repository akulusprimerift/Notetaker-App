...
(async()=>{
  ...
  try{
    ...
    let original=await page.locator('.note-pages-flow').textContent();
    ...
    await page.waitForFunction(()=>document.querySelector('[aria-current=page]')?.textContent==='2');
        const oldCount=await page.locator('.note-page-numbers button').count();
    notes.revision={...notes.revision,id:'revision2',revision:2,content:{...notes.revision.content,blocks:[...notes.revision.content.blocks,{id:'appended',kind:'explanation',topic:'Newly saved explanation',passages:[{id:'appended-passage',evidence_kind:'lecture_paraphrase',text:'Appended notes. '.repeat(200),sources:[]}]}]}};
    socket.send(JSON.stringify({schema_version:1,kind:'heartbeat',cursor:0}));
    await page.waitForFunction(count=>document.querySelectorAll('.note-page-numbers button').length>count,oldCount);
    assert.equal(await page.locator('.note-page-numbers [aria-current=page]').textContent(),'2','new notes preserve current section');
    original=await page.locator('.note-pages-flow').textContent();
