...
Current priority (2026-09-17): **7.3 — Learning tools** remains active. Generated question/flashcard sets, protected edits/history, self-assessments and a separate four-dimension quality rubric are implemented. An installed local model was evaluated on synthetic CS, biology, physics and history fixtures; all four returned structurally valid sets, but worked-step coverage remains a quality gap. See [evaluation and human qualification](ai/phase-7-3-learning-evaluation.md). Standalone Windows distribution is the next requested delivery priority, with earlier qualification gaps explicitly retained.
...
| 6.8 / M08 | Implemented with open qualification gates: user-selected Electron Windows rebuild with a cleaner lecture workspace, native-feature parity in the shared React UI, local model discovery and course/lecture data controls. | Electron installer and installed launch are the delivery path. Complete UI flow, service lifecycle, clean-machine, hardware, endurance and restore gates. |
...
Phase **7.1 — Note usefulness** is now the only active implementation phase, following the committed theme/account repairs and full exposed-catalog refresh/disconnect follow-up. Mark Important, Catch Me Up and course terminology hints are implemented; see [behavior and engineering evidence](implementation/phase-7-1.md). Human usefulness qualification remains open. Continue selected Phase 7 work, then standalone Windows distribution, then macOS development. Keep existing Electron/Docker data and earlier qualification gaps; do not claim M08 release readiness from these usability changes.
