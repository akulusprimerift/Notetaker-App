...
This roadmap reflects the current priority: the user-selected LLM automatically writes excellent notes from lecture transcripts, for any subject and content-heavy course. CS is a test anchor, not a product restriction. Manual editing is optional review. The user accepts the [M05 live workflow](implementation/phase-6-m05.md). The active [M06 increment](implementation/phase-6-m06.md) improves speech latency, contextual batching and streaming, removes redundant preference dropdowns, and implements protected editing/regeneration. Actual model performance, semantic topic quality and Windows release qualification remain open.
...
| 6.6 / M06 | Active: protected editing and regeneration implemented, with live pipeline improvements. | See [M06 behavior/evidence](implementation/phase-6-m06.md) and [session transfer](../SESSION_TRANSFER.md) for executed checks. Qualify actual model performance and educational quality separately. |
| 6.7–6.8 / M07–M08 | Planned. | Finalization/data control then Windows delivery follow this increment. |
