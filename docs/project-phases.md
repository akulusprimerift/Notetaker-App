...
This roadmap reflects the current priority: the user-selected LLM automatically writes excellent notes from lecture transcripts, especially for content-heavy courses. Manual editing is optional review. A bounded saved-transcript implementation now exists in [M04](implementation/phase-6-m04.md); long-lecture topic processing remains next.
