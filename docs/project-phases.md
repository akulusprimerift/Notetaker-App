...
| 6.8 / M08 — Full lecture release testing | Use the entire workflow for a dense lecture with readable notes, accessible controls and a documented recovery/backup process. | Endurance and integration harness; supported-environment record; measured diagnostics; coordinated database/object backup and restore with deletion reconciliation. Fix demonstrated bottlenecks without expanding the stack by default. | All 32 acceptance scenarios and six release gates have evidence, including an independently annotated 45–60 minute CS lecture, note-quality review, accessibility, interruptions and restore checks. |
...
- **Deployment:** retain the existing local Docker Compose foundation. Add infrastructure only for a requirement or measured limitation. Do not introduce vector search, caches, orchestration clusters or extra service boundaries merely because a later phase might use them.
...
| 6.3 / M03 | Next implementation phase; transcription is not built. | Establish the speech worker and versioned transcript path using synthetic speech under the current testing preference, then integrate source playback and corrections. |
