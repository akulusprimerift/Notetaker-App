...
Extract the entire `Notetaker-Windows-x64.zip` into a folder and open `Notetaker.exe`. Keep the `_internal` folder and `NotetakerService.exe` next to it. Windows x64 is required. This native Qt Widgets application contains no Chromium, Electron or WebView engine. Docker, Python, PowerShell, Ollama and a browser are not required on the receiving computer.
